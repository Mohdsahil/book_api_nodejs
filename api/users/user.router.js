...
router.post("/login", (req, res) => {
  ...
  pool.query(
    ...
    (err, result, fields) => {
      ...
      var token = jwt.sign({ id: result.insertedId }, process.env.TOKEN_KEY, {
        expiresIn: 86400,
      });
      return res.status(200).json({
        success: 1,
...
router.get("/:name:type", (req, res) => {
  ...
  pool.query(
    `select * from books where book_name like ? || book_type like ?`,
    [name + "%", type + "%"],
    (err, result, fields) => {
      if (err) {
        console.log(err);
...
router.post("/readed", checktoken, (req, res) => {
  ...
  pool.query(
    `INSERT INTO readinghistory(book_id, user_id, read_time_count, rating, review) values(?,?,?,?,?)`,
    [data.book_id, data.user_id, data.time_count, data.rating, data.review],
    (err, result, fields) => {
      if (err) {
        console.log(err);
        return res.status(500).json({
